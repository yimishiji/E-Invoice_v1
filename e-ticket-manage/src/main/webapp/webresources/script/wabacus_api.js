...
{
	if(editedElementObj==null) {return;}else{
		$(editedElementObj).css("border","1px solid #f8ac59");
		$(editedElementObj).css("box-shadow","0 0 5px rgba(0, 0, 0, 0.3)");
	}
}
...
function changeInputboxStyleByJsValidateError(elementObj)
{
	...
	if(inputboxObj!=null)
	{//如果取到输入框对象，则改变输入框样式
		$(inputboxObj).css("border","1px solid #a94442");
		$(inputboxObj).css("box-shadow","0 0 5px rgba(0, 0, 0, 0.3)");
	}else
	{//如果没有取到，则改变父标签样式
...
{
	layer.alert(message, {title: false,icon: 1,time: 3000});
...
function wx_warn(message,paramsObj)
{
	...
	layer.alert(message, {title: false,icon: 2,time: 3000});
	/*if(paramsObj==null) paramsObj=new Object();
	if(!isExistPromptObj())
	{
		alert(message);
...
function wx_confirm(message,title,width,height,okHandler,cancelHandler)
{
	...
	layer.confirm(message, {icon: 3, title:false}, function(index){
		okHandler();
		layer.close(index);
	});
...
function wx_winpage(url,paramsObj,beforeCallBackMethod,callbackMethodParams)
{
	...
	var index = layer.open({
	  ...
	  maxmin: false,
	  content: url,
	  end:function(index){ refreshComponentDisplay(); } 
	});
	layer.full(index);*/
...
function wx_tip(message,element,paramsObj)
{
	...
	//$(".cls-data-td-editdetail").tooltip(options);
	changeInputboxStyleByJsValidateError(element);
	$(element).popover(options);
	$(element).popover("show").on('shown.bs.popover', function () {
		$("body").click(function(){
